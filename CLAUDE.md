...
- **Provenance:** each tab is generated by a Python script in the sibling `../getting-data` folder and pasted in — `days_generate.py`→`days`, `weather_generate.py`→`brasov`/`sibiu-weather`, `eurron_generate.py`→`euro` (FX), `commodities_generate.py`→`commodities`. `_generate_all.py` runs them all. All series start 2020-01-01 and are forward-filled to every calendar day (weekends/holidays carry the last value).
