...
- **Events panel:** ≤10-day ranges → Wikipedia Current Events daily digest; longer → year-article "Events" (curated majors), filtered to range. Heuristically scored (topic weight + impact keywords + casualties + coverage), tiered major/notable/minor, "Important only" hides minor. Clicking a chart point focuses the panel on that point's period. Recharts v3 `onClick` gives `activeIndex` (NOT v2's `activePayload`). The curated markers drawn on the chart are also **named directly beneath it** by `MarkerLegend` (Dashboard), split into "Event ranges" (shaded bands) and "Events" (single-unit markers), colour-dotted by tier.
