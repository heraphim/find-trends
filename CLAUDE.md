...
- **Generation:** `.github/workflows/generate-events.yml` — **manual (`workflow_dispatch`) only**. Runs Claude Code (`claude -p`, prompt in `.github/prompts/generate-events.md`) on the user's **Claude subscription** via the `CLAUDE_CODE_OAUTH_TOKEN` secret (from `claude setup-token`; not API-key billing), then opens a **PR for human review** via `peter-evans/create-pull-request`. The Claude step runs with `--output-format json`; a follow-up step parses the token usage + `total_cost_usd` into a **cost report** shown in the PR body and the Actions run summary. The prompt forbids modifying existing rows — it only backfills empty cells and appends new events. Deliberately not scheduled and never auto-committed: AI output is non-deterministic and can hallucinate, so a human gates every change to this correlation-input data. First run needs the token secret + the repo's "Allow GitHub Actions to create and approve pull requests" setting.
